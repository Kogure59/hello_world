...
SETUP.md では CircleCI のセットアップやジョブの実行、それに伴う AWS 上での設定などを詳しく説明しています。

## Docker を用いたローカル開発環境

Windows 環境で Ansible を実行するために Docker を活用しました。取り組みの詳細は [lecture13.md](reports/lecture13.md) を参照してください。

Amazon Linux 2 をベースイメージとして Python3 / pip / SSH クライアント / Ansible をインストールした実行環境を構築し、ホストの作業ディレクトリをコンテナにマウントすることで、ローカルでの編集内容をコンテナ内にそのまま反映できるようにしました。構文チェック → ドライラン → デバッグ実行 → 本実行という段階的な検証フローを組み、Windows に直接 Ansible をインストールせずに安定した開発・テストサイクルを実現しました。
